...
bool Proveedor::Anadir()
{
    ...
    if(nid != -1)
    {
        ...
        if(succes)
            TimedMessageBox::Box(qApp->activeWindow(),tr("Proveedor insertado corectamente"));
        else
...
bool Proveedor::Recuperar(int id)
{
    ...
    {
        if(error.isEmpty())
            TimedMessageBox::Box(qApp->activeWindow(),QObject::tr("No se encuentra el proveedor"));
        else
...
bool Proveedor::Next()
{
    ...
    {
        if(error.isEmpty())
            TimedMessageBox::Box(qApp->activeWindow(),QObject::tr("Se alcanzó el final del archivo"));
        else
            QMessageBox::warning(qApp->activeWindow(),QObject::tr("Gestión Proveedores"),
                                 QObject::tr("Problema en la BD no se puede recuperar el proveedor:\n")+
                                        error ,QObject::tr("Ok"));
...
void Proveedor::Guardar()
{
   // QScopedPointer<QSqlQuery>qProveedor(new QSqlQuery(Configuracion_global->groupDB));
    ...
    if(!queryProveedor.exec()){
        ...
        TimedMessageBox::Box(qApp->activeWindow(),QObject::tr("Los datos se han guardado corectamente"));
    }
}
...
{        
    ...
    if(SqlCalls::SqlDelete("proveedores",Configuracion_global->groupDB,QString("id = %1").arg(nid),error))
        TimedMessageBox::Box(qApp->activeWindow(),tr("Proveedor borrado con éxito"));
    else
