...
{
    qEmpresa = QSqlQuery(Configuracion_global->groupDB);
...
void Empresa::Recuperar(QString cSQL)
{
    ...
    else 
	{
		...
		{
            TimedMessageBox::Box(qApp->activeWindow(),
...
void Empresa::Recuperar(QString cSQL, int nProcede)
{
    ...
    if (!qEmpresa.exec(cSQL))
        ...
        if(qEmpresa.next()) {
            ...
            if (nProcede == 1)
                TimedMessageBox::Box(qApp->activeWindow(),QObject::tr("No hay más empresas: Se ha llegado al final del fichero"));
            else
                TimedMessageBox::Box(qApp->activeWindow(),QObject::tr("No hay más empresas: se ha llegado al inicio del fichero"));
