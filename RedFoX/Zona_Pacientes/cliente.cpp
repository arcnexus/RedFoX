...
bool Cliente::Anadir()
{
    ...
    else
    {
        ...
        if(succes)
            TimedMessageBox::Box(qApp->activeWindow(),"Cliente insertado Corectamente");
        else
            QMessageBox::critical(qApp->activeWindow(),"error al insertar:", error);
        return succes;
    }
...
//        TimedMessageBox::Box(qApp->activeWindow(),tr("Se ha creado el cliente en la web"));
...
void Cliente::Borrar(int id_cliente)
{
                          ...
                          tr("No"),tr("Si")) == QMessageBox::Accepted)
    {
        ...
        {
            Configuracion_global->groupDB.commit();
            TimedMessageBox::Box(qApp->activeWindow(),tr("Borrado corectamente"));
            this->clear();
        }
        else
        {
