...
void FrmPresupuestosCli::on_btnAnterior_clicked()
{
    ...
    {
        TimedMessageBox::Box(this,tr("Se ha llegado al principio del archivo.\n"
                                                          "No hay mas presupuestos disponibles"));
        VaciarCampos();
        ui->btnBorrar->setEnabled(false);
        ui->btnEditar->setEnabled(false);
...
void FrmPresupuestosCli::on_btnGuardar_clicked()
{
    ...
    if(succes)
    {
        TimedMessageBox::Box(this,tr("Guardado con éxito"));
        BloquearCampos(true);
        emit unblock();
    }
    else
...
void FrmPresupuestosCli::on_btnBorrar_clicked()
{
                              ...
                              tr("&Continuar"))== QMessageBox::Accepted)
    {
        ...
        if(succes)
        {
            ...
            TimedMessageBox::Box(this,tr("Borrado con exito"));
        }
        else
        {
            Configuracion_global->empresaDB.rollback();
...
void FrmPresupuestosCli::convertir_epedido()
{
    if(ui->txtpedido->text() =="0" && ui->txtalbaran->text() =="0" && ui->txtfactura->text() =="0")
    {
        ...
        else //ALL OK
        {
            ...
            if(updated)
            {
                ...
                TimedMessageBox::Box(this,tr("Se ha creado el pedido num:")+num);
    ...
    else //ALREADY CONVERTED
    {
        if(ui->txtfactura->text() != "0")
            TimedMessageBox::Box(this,tr("Este presupuesto ya ha sido pasado a factura: %1").arg(oPres->factura));
        else if(ui->txtalbaran->text() !="0")
            TimedMessageBox::Box(this,tr("Este presupuesto ya ha sido pasado a albarán: %1").arg(oPres->albaran));
        else
            TimedMessageBox::Box(this,tr("Este presupuesto ya ha sido pasado a pedido: %1").arg(oPres->pedido));
    }
}
    ...
    if(ui->txtpedido->text() =="0" && ui->txtalbaran->text() =="0" && ui->txtfactura->text() =="0")
    {
        ...
        else
        {
            ...
            {               
                ...
                if(commited)
                    TimedMessageBox::Box(this,tr("Se ha creado el albarán num: ")+QString("%1/%2").arg(serie).arg(num));
                else
    ...
    if(dlg_print.exec() == dlg_print.Accepted)
    {
        ...
        if(ok)
        {
            ...
            case 1: // Impresora
                ...
                break;
            case 2: // email
                Configuracion::EviarMail(report,parametros_sql,parametros,pdfname,dlg_print.get_email(),oClientePres->nombre_fiscal,asunto,texto);
                break;
            case 3: // PDF
...
void FrmPresupuestosCli::on_chkrecargo_equivalencia_toggled(bool checked)
{
    ...
    for(auto i = 0; i<modelLineas->rowCount();++i)
    {
        ...
        if(iva_art == oPres->porc_iva1)
            porc_re_art = oPres->porc_rec1;
        if(iva_art == oPres->porc_iva2)
            porc_re_art = oPres->porc_rec2;
        if(iva_art == oPres->porc_iva3)
            porc_re_art = oPres->porc_rec3;
        if(iva_art == oPres->porc_iva4)
            porc_re_art = oPres->porc_rec4;

        lin["porc_rec"] = porc_re_art;
        lin["rec"] = r.value("total").toDouble() * (porc_re_art/100.0);

        if(!SqlCalls::SqlUpdate(lin,"lin_pre",Configuracion_global->empresaDB,QString("id=%1").arg(r.value("id").toInt()),error))
        {
