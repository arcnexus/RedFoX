...
void FrmPedidos::on_btnGuardar_clicked()
{
    ...
    if(oPedido->GuardarPedido(oPedido->id))
    {
        ...
        TimedMessageBox::Box(this,tr("Pedido guardado con éxito"));
        LLenarCampos();
        BloquearCampos(true);
        emit unblock();
    }
...
void FrmPedidos::on_btn_borrar_clicked()
{
                              ...
                              tr("Borrar"))== QMessageBox::Accepted)
    {
        ...
        if(succes)
        {
            TimedMessageBox::Box(this,tr("Pedido borrado con éxito"));
            VaciarCampos();
            oPedido->id=-1;
...
void FrmPedidos::convertir_en_albaran()
{
    ...
    else
    {
        if(ui->txtcNumFra->text() != "0")
            TimedMessageBox::Box(this,tr("Este pedido ya ha sido pasado a factura: %1").arg(oPedido->factura));
        else if(ui->txtalbaran->text() !="0")
            TimedMessageBox::Box(this,tr("Este pedido ya ha sido pasado a albarán: %1").arg(oPedido->albaran));
    }
}
