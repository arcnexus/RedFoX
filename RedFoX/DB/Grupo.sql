...
CREATE OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vista_art_prov` AS select `articulos`.`id` AS `id_articulo`, `articulos_prov_frec`.`id_proveedor` AS `id_proveedor`, `articulos`.`codigo` AS `codigo`, `articulos`.`codigo_barras` AS `codigo_barras`, `articulos`.`codigo_fabricante` AS `codigo_fabricante`, `articulos_prov_frec`.`codigo` AS `codigo_proveedor`, `articulos`.`descripcion_reducida` AS `descripcion_reducida`, `articulos`.`tipo_iva` AS `tipo_iva`,`articulos`.`id_tipos_iva` AS `id_tipos_iva`, `articulos_prov_frec`.`pvd` AS `coste`, `articulos`.`stock_fisico_almacen` AS `stock_fisico_almacen`, `articulos`.`stock_real` AS `stock_real`, `articulos`.`cantidad_pendiente_recibir` AS `cantidad_pendiente_recibir`, `articulos`.`fecha_prevista_recepcion` AS `fecha_prevista_recepcion`, `articulos`.`unidades_reservadas` AS `unidades_reservadas`, `articulos`.`controlar_stock` AS `controlar_stock` from (`articulos_prov_frec` left join `articulos` on(`articulos`.`id` = `articulos_prov_frec`.`id_articulo`))
