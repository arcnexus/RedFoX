...
CREATE OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `@grupo@`.`vista_art_prov` AS select `@grupo@`.`articulos`.`id` AS `id_articulo`,`@grupo@`.`articulos_prov_frec`.`id_proveedor` AS `id_proveedor`,`@grupo@`.`articulos`.`codigo` AS `codigo`,`@grupo@`.`articulos`.`codigo_barras` AS `codigo_barras`,`@grupo@`.`articulos`.`codigo_fabricante` AS `codigo_fabricante`,`@grupo@`.`articulos_prov_frec`.`codigo` AS `codigo_proveedor`,`@grupo@`.`articulos`.`descripcion_reducida` AS `descripcion_reducida`,`@grupo@`.`articulos`.`tipo_iva` AS `tipo_iva`,`@grupo@`.`articulos`.`id_tipos_iva` AS `id_tipos_iva`,`@grupo@`.`articulos_prov_frec`.`pvd` AS `coste`,`@grupo@`.`articulos`.`stock_fisico_almacen` AS `stock_fisico_almacen`,`@grupo@`.`articulos`.`stock_real` AS `stock_real`,`@grupo@`.`articulos`.`cantidad_pendiente_recibir` AS `cantidad_pendiente_recibir`,`@grupo@`.`articulos`.`fecha_prevista_recepcion` AS `fecha_prevista_recepcion`,`@grupo@`.`articulos`.`unidades_reservadas` AS `unidades_reservadas`,`@grupo@`.`articulos`.`controlar_stock` AS `controlar_stock` from (`@grupo@`.`articulos_prov_frec` left join `@grupo@`.`articulos` on(`@grupo@`.`articulos`.`id` = `@grupo@`.`articulos_prov_frec`.`id_articulo`));
