...
void Interconsulta::AnadirInterconsulta(int id_episodio,int id_paciente)
{
    ...
    if(qInterconsulta->exec())
        TimedMessageBox::Box(qApp->activeWindow(),tr("Se ha añadido el registro corectamente"));
    else
        TimedMessageBox::Box(qApp->activeWindow(),tr("No se ha podido añadir el registro corectamente"));
