...
void AlbaranProveedor::Cargar(QSqlQuery query)
{
    ...
    else
    {
        ...
        {
        case 1: // siguiente
            t = new TimedMessageBox(qApp->activeWindow(),tr("Se ha llegado al último albarán"));
            break;
        case 2: // anterior
            t = new TimedMessageBox(qApp->activeWindow(),tr("Se ha llegado al primer albarán"));
            break;
        default:
            t = new TimedMessageBox(qApp->activeWindow(),tr("No se encontró el albarán"));
            break;
        }
    }
